...
class RefugeeLogicTest {
    ...
    @Test
    @DirtiesContext
    void testRefugeeRegistration() {
        // setup
        RefugeeRegisterDTO refugeeRegisterDTO = new RefugeeRegisterDTO(refugee1.getEmail(),refugee1.getPassword(),refugee1.getGender(),refugee1.getNationality(),refugee1.getFirstName(),refugee1.getMiddleName(),refugee1.getLastName(),refugee1.getDateOfBirth(),refugee1.getFamilySize(),refugee1.getDescription(),"");
        Mockito.when(refugeeCommunication.getRefugeeByEmail(refugee1.getEmail())).thenReturn(Optional.empty());
        Mockito.when(refugeeCommunication.createRefugee(refugee1)).thenReturn(refugee1);
        // action
        underTest.registerRefugee(refugeeRegisterDTO);
        // testing
        ...
        Assertions.assertEquals(argCaptor.getValue().toString(), refugee1.toString());
    }

    @Test
    @DirtiesContext
    void testRefugeeLogin()
    {
        // setup
        LoginDTO loginDTO = new LoginDTO(refugee1.getEmail(),refugee1.getPassword());
        when(refugeeCommunication.getRefugeeByEmail(refugee1.getEmail())).thenReturn(Optional.of(refugee1));
        // action
        RefugeeDTO result = underTest.loginRefugee(loginDTO);
        // testing
    ...
    @Test
    @DirtiesContext
    void testRefugeeRegistration_WithExistingEmail()
    {
        // setup
        RefugeeRegisterDTO refugeeRegisterDTO = new RefugeeRegisterDTO(refugee1.getEmail(),refugee1.getPassword(),refugee1.getGender(),refugee1.getNationality(),refugee1.getFirstName(),refugee1.getMiddleName(),refugee1.getLastName(),refugee1.getDateOfBirth(),refugee1.getFamilySize(),refugee1.getDescription(),"");
        Mockito.when(refugeeCommunication.getRefugeeByEmail(refugee1.getEmail())).thenReturn(Optional.of(refugee1));
        // action
        RefugeeDTO result = underTest.registerRefugee(refugeeRegisterDTO);
        // testing
        assertEquals("Refugee with email "+refugee1.getEmail()+" already exists.",result.getErrorMessage());
        verify(refugeeCommunication, never()).createRefugee(any());
    }

    @Test
    @DirtiesContext
    void testRefugeeLogin_WithoutExistingUser()
    {
        // setup
        LoginDTO loginDTO = new LoginDTO(refugee1.getEmail(),refugee1.getPassword());
        ...
        // testing
        verify(refugeeCommunication).getRefugeeByEmail(refugee1.getEmail());
        assertEquals("Refugee with email " + refugee1.getEmail() + " doesn't exist.",result.getErrorMessage());
    }

    @Test
    @DirtiesContext
    void testRefugeeLogin_WithoutMatchingPassword()
    {
        // setup
        LoginDTO loginDTO = new LoginDTO(refugee1.getEmail(),"Wrong password");
        when(refugeeCommunication.getRefugeeByEmail(refugee1.getEmail())).thenReturn(Optional.of(refugee1));
        // action
        RefugeeDTO result = underTest.loginRefugee(loginDTO);
        // testing
        verify(refugeeCommunication).getRefugeeByEmail(refugee1.getEmail());
        assertEquals("Password is incorrect",result.getErrorMessage());
    }
