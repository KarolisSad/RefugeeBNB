...
class HostLogicTest {
    ...
    @Test
    @DirtiesContext
    void testHostRegistration() {
         // setup
        HostRegisterDTO hostRegisterDTO = new HostRegisterDTO(host1.getEmail(),host1.getPassword(),host1.getGender(),host1.getNationality(),host1.getFirstName(),host1.getMiddleName(), host1.getLastName(),host1.getDateOfBirth(),"");
        Mockito.when(hostCommunication.getHostByEmail(host1.getEmail())).thenReturn(Optional.empty());
        Mockito.when(hostCommunication.createHost(host1)).thenReturn(host1);
         // action
        underTest.registerHost(hostRegisterDTO);
         // testing
        ...
        Assertions.assertEquals(argCaptor.getValue().toString(), host1.toString());
    }

    @Test
    @DirtiesContext
    void testHostRegistration_WithExistingEmail()
    {
         // setup
        HostRegisterDTO hostRegisterDTO = new HostRegisterDTO(host1.getEmail(),host1.getPassword(),host1.getGender(),host1.getNationality(),host1.getFirstName(),host1.getMiddleName(), host1.getLastName(),host1.getDateOfBirth(),"");
        Mockito.when(hostCommunication.getHostByEmail(host1.getEmail())).thenReturn(Optional.of(host1));
         // action
        HostDTO result = underTest.registerHost(hostRegisterDTO);
         // testing
        assertEquals("Host with email "+host1.getEmail()+" already exists.",result.getErrorMessage());
        verify(hostCommunication, never()).createHost(any());
    }
    ...
    void testHostLogin()
    {
        ...
        when(hostCommunication.getHostByEmail(host1.getEmail())).thenReturn(Optional.of(host1));
         // action
        HostDTO result = underTest.loginHost(loginDTO);
         // testing
    ...
    @Test
    @DirtiesContext
    void testHostLogin_WithoutExistingUser()
    {
        // setup
        LoginDTO loginDTO = new LoginDTO(host1.getEmail(),host1.getPassword());
        ...
        // testing
        verify(hostCommunication).getHostByEmail(host1.getEmail());
        assertEquals("Host with email " + host1.getEmail() + " doesn't exist.",result.getErrorMessage());
    }

    @Test
    @DirtiesContext
    void testHostLogin_WithoutMatchingPassword()
    {
        // setup
        LoginDTO loginDTO = new LoginDTO(host1.getEmail(),"Wrong password");
        when(hostCommunication.getHostByEmail(host1.getEmail())).thenReturn(Optional.of(host1));
        // action
        HostDTO result = underTest.loginHost(loginDTO);
        // testing
